...
$year = date('Y');
    ...
    $refID = $mysqli->query("SELECT id,COUNT(remark) AS sekolahPantau FROM dataSekolah WHERE remark = 'observe'");
    $RID = mysqli_fetch_assoc($refID);

    $refID2 = $mysqli->query("SELECT id,SUM(CASE WHEN bukuStok > 0 THEN bukuStok ELSE 0 END) AS bukuStok FROM rekodPemantauan");
    $RID2 = mysqli_fetch_assoc($refID2);

    $refID3 = $mysqli->query("SELECT id,SUM(CASE WHEN bukuLebihan > 0 THEN bukuLebihan ELSE 0 END) AS bukuLebihan FROM rekodPemantauan");
    $RID3 = mysqli_fetch_assoc($refID3);

    $refID4 = $mysqli->query("SELECT id,COUNT(negeri) AS belumPantau FROM dataSekolah WHERE remark NOT LIKE 'observe'");
...
<div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-info">
              <div class="inner">
                <h3 style="font-family: 'Anton', sans-serif;"><?php if (!empty($RID['id'])){echo $RID['sekolahPantau'];}else{echo 0;}?></h3>
